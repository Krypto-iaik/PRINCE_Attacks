...
void attack3SubSpaceAttack(unsigned char initialKey[16], unsigned char initialKey2[16], int Ntest)
{
    ...
    for(i=0;i<16;i++)
    {
        ...
        moment = byteTransformation(c[0][1] ^ indice1 ^ alpha[1]);
        numeroSBOX++;

        for(j = 0; j<Ntest;j++)
        {
            state[j] = check2(moment, c[j][1], indice1, alpha[1], values);
            numeroSBOX = numeroSBOX + 1;
        }
        ...
        for(k=0;k<16;k++)
        {
            ...
            moment = byteTransformation(c[0][0] ^ indice ^ alpha[0]);
            numeroSBOX++;

            do
            {
                flag0 = check2(moment, c[j][0], indice, alpha[0], values);
                numeroSBOX = numeroSBOX + 1;

                if(flag0 != (state[j] & 0x7))
                   flag = 1;

                j++;

            }while((flag==0)&&(j<Ntest));
        ...
        //Attack nibble in position 2nd
        cont = 0;
        for(k=0;((k<16)&&(((int) indice0[0]) <16));k++)
        {
            indice = (word8) k;
            ...
            moment = byteTransformation(c[0][2] ^ indice ^ alpha[2]);
            numeroSBOX++;

            do
            {
                flag0 = check2(moment, c[j][2], indice, alpha[2], values);
                numeroSBOX = numeroSBOX + 1;

                if(flag0 != (state[j] & 0xd))
                    flag = 1;
        ...
        //Attack nibble in position 3rd
        cont = 0;
        for(k=0;((k<16)&&(((int) indice2[0]) <16)&&(((int) indice0[0]) <16));k++)
        {
            indice = (word8) k;
            ...
            values = 0xb;
            moment = byteTransformation(c[0][3] ^ indice ^ alpha[3]);
            numeroSBOX++;

            do
            {
                flag0 = check2(moment, c[j][3], indice, alpha[3], values);
                numeroSBOX = numeroSBOX + 1;

                if(flag0 != (state[j] & 0xe))
                    flag = 1;
        ...
        if((((int) indice0[0]) <16)&&(((int) indice2[0]) <16)&&(((int) indice3[0]) <16))
        {
            printf("FIRST Nibble\n");
            cont = 0;
            while((cont<16)&&(((int) indice0[cont]) <16))
            {
                printf("\tCorrect values: %x - Found Values: %x", (keyy22[0]^initialKey2[0]), indice0[cont]);
                if((keyy22[0]^initialKey2[0])==indice0[cont])
                    printf(" - RIGHT NIBBLE\n");
                else
                    printf("\n");
                cont++;
            ...
            printf("\tCorrect values: %x - Found Values: %x", (keyy22[1]^initialKey2[1]), indice1);
            if((keyy22[1]^initialKey2[1])==indice1)
                    printf(" - RIGHT NIBBLE\n");
                else
                    printf("\n");
            ...
            {
                printf("\tCorrect values: %x - Found Values: %x", (keyy22[2]^initialKey2[2]), indice2[cont]);
                if((keyy22[2]^initialKey2[2])==indice2[cont])
                    printf(" - RIGHT NIBBLE\n");
                else
                    printf("\n");
                cont++;
            ...
            {
                printf("\tCorrect values: %x - Found Values: %x", (keyy22[3]^initialKey2[3]), indice3[cont]);
                if((keyy22[3]^initialKey2[3])==indice3[cont])
                    printf(" - RIGHT NIBBLE\n");
                else
                    printf("\n");
                cont++;
    ...
    printf("The theoretical value for this step is of 4*2^(7.3)=631 S-Box look-ups for 5 CP.\n\n");
    printf("(The term 4 * is due to the fact that this program is not optimized, using the way described in the paper. The missing ");
...
/*This program attack 3 rounds of Prince using the attack described in App. D.2, that is the subspace trail attack.
...
int main()
{
    ...
    printf("TRUNCATED ATTACK on 3 ROUNDS of PRINCE EXPLOITING THE RELATIONSHIPS AMONG THE NIBBLES (App. D.2).\n\n");
    ...
    for(i=0;i<numberTest;i++)
    {
        ...
        attack3SubSpaceAttack(FXkey, KeyPrinceCore, numberPlaintext);
        printf("\n");
    }
